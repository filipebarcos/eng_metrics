...
func FetchPullRequests(ctx context.Context, _message shared.PubSubMessage) error {
	...
	for _, repo := range ghConfig.Repositories {
		query, variables := gh.PullRequestsQuery(ghConfig.Organization, repo)
		...
		for {
			...
			for _, pr := range query.Repository.PullRequests.Nodes {
				_, err = db.CreatePullRequest(dbClient, models.BuildPullRequest(pr, repository.ID))
				if err != nil {
